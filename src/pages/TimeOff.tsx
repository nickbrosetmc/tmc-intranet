    ...
    <div className="w-full max-w-4xl space-y-4">
      <header className="border-b border-tmc-gold/40 pb-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your time-off requests and balances from Connecteam.
        </p>
      </header>

      {!isTeam ? (
        <div className="text-sm text-muted-foreground">
          Sign in as a team member to see your time-off requests.
        </div>
      ) : (
        <ComingSoon />
      )}
  ...
  return (
    <div className="rounded-lg border bg-card p-12 text-center space-y-3">
      ...
      <p className="text-sm text-muted-foreground max-w-md mx-auto">
        Connecteam time-off integration is being wired up. Once it's live,
        you'll see your pending requests, balances, and (for admins) approve
        or deny pending requests with one click.
      </p>
    </div>
  );
}
